...
const User = (props:any) => {
    ...
    let {profile, editMode, setEditMode, setNewContact, setUserProfile} = props;

    let isOwner = false;
    if (!!props.isAuth && profile && props.userInfo.userID === profile.userId) {
        isOwner = true
    }

    let handleSubmit = (values:React.FormEvent) => {
        setUserProfile(values);
        setNewContact(values);
    }

    if (editMode) {
        return <UserReduxForm onSubmit={handleSubmit}  {...props}  initialValues={profile}/>
    }

    let onEditClick = (e:React.MouseEvent) => {
        setEditMode(true)
    }
    // let onContactCange = (newValue, contactKey) => {
    //
    //     updateContact(newValue, contactKey)
    // };
    // let onSaveClick = () => {
    //     setNewContact();
    //     setEditMode(false)
